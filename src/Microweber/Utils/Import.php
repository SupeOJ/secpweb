...
class Import
{
    ...
    public function move_uploaded_file_to_import($params)
    {
        only_admin_access();
        if (!isset($params['src'])) {
            return array('error' => '您还没有向文件提供SRC。');
        }
        ...
        if (is_file($check)) {
            ...
            if (copy($check, $here . $fn)) {
                ...
                return array('success' => "$fn 已移动！");
            } else {
                return array('error' => '移动上传文件时出错！');
            }
        } else {
            return array('error' => '未找到上传的文件！');
        }
    }
    ...
    public function delete($params)
    {
        ...
        if ($id == null) {
            return array('error' => '您没有提供要删除的文件名。');
        }
    ...
    public function download($params)
    {
        ...
        // Check if the file has needed args
        if ($id == null) {
            return array('error' => '您没有提供下载的文件名。');
        ...
        $filename = str_replace('..', '', $filename);
        if (!is_file($filename)) {
            return array('error' => '您尚未提供下载的现有文件名。');
    ...
    public function restore($params)
    {
        ...
        if ($id == null) {
            return array('error' => '您没有提供要还原的文件。');
            die();
        }
        $id = str_replace('..', '', $id);
        ...
        if (!is_file($filename)) {
            return array('error' => '您还没有提供恢复的现有备份。');
        } else {
    ...
    public function import_file($filename)
    {
        ...
        if (method_exists($this, $import_method)) {
            ...
            return array('error' => "找不到导入 $ext 文件的方法。");
        }
    }
...
// Now that we're at the right depth, hop to the next "[item name]" until the end of tree/file.
        ...
        return array('success' => ($i) . ' XML项目将被导入');
    }
    ...
    public function queue_import_xls($filename)
    {
        ...
        if ($err == true) {
            return array('error' => '无法联系secpweber远程服务器来解析Excel文件。请尝试上传一个CSV文件。');
        } else {
            if (isset($result['result'])) {
                $url = $result['result'];
    ...
    public function queue_import_csv($filename)
    {
        ...
        if (empty($head) or empty($csv)) {
            return array('error' => '无法正确解析CSV文件。');
        }
        $rows = array();
        $i = 0;
        foreach ($csv as $row) {
    ...
    public function batch_save($content_items)
    {
        ...
        return array('success' => count($content_items) . ' 项目预定导入。');
    }
