...
class MenuManager
{
    ...
    public function menu_item_save($data_to_save)
    {
        ...
        $data_to_save['table'] = $table;
		if(!isset($data_to_save['item_type'])){
          $data_to_save['item_type'] = 'menu_item';
		}
		if(isset($data_to_save['text'])){
		$data_to_save['icon'] = $data_to_save['text'];
		}
        $save = $this->app->database_manager->save($table, $data_to_save);
...
/*                    $new_menu = $this->menu_create('id=0&title=' . $params['title']);
    ...
    public function get_pdf($params,$contentps)
    {
        $table = $this->tables['menus'];
        ...
        //$table = MODULE_DB_SHOP_ORDERS;
		if(!empty($contentps)){
			$params['userid'] = $contentps['userid'];
			$params['content_id'] = $contentps['contentid'];
		}
        $params['table'] = $table;
        $params['item_type'] = 'pdf';
        //$params['debug'] = 'menu';
...
/*                    $new_menu = $this->menu_create('id=0&title=' . $params['title']);
    ...
    public function get_pdfid($id)
    {
        ...
        $params['item_type'] = 'pdf';
        $params['orderby'] = 'position';
		if(!empty($id['content_id'])){
		  $params['content_id'] = $id['content_id'];
		}
		if(!empty($id['id'])){
		  $params['id'] = $id['id'];
		}
	...
	public function get_dropdown_iframe($menu_id)
	{
        $table = $this->tables['menus'];
        $params['table'] = $table;
		...
		if(!empty($menus)){
			foreach($menus as $k1 => $v1){
				$menu_arr1[$v1['id']] = $v1['url'];
		        $params['parent_id'] = $v1['id'];
					
                $menus1 = $this->app->database_manager->get($params);
			 	
				if(!empty($menus1)){
				
					foreach($menus1 as $k2 =>$v2){
				        $menu_arr2[$v2['id']] = $v2['url'];
						$params['parent_id'] = $v2['id'];
						$menus2 = $this->app->database_manager->get($params);
						if(!empty($menus2)){
				         	foreach($menus2 as $k3 =>$v3){
								$menu_arr3[$v3['id']] = $v3['url'];
								$params['parent_id'] = $v3['id'];
								$menus3 = $this->app->database_manager->get($params);
								if(!empty($menus3)){
									foreach($menus3 as $k4 =>$v4){
									$menu_arr4[$v4['id']] = $v4['url'];
									$params['parent_id'] = $v4['id'];
									}
								}
		...
		$data  = $menu_arr1+$menu_arr2+$menu_arr3+$menu_arr4;
...
//                            } else {
    ...
    //获取所有启用符合条件数据
    public function get_menus_data($params = false)
    {
        $table = $this->tables['menus'];
        if (is_string($params)) {
            $params = parse_params($params);
    ...
    public function get_dropdown_menusarr($menu_id, $maxdepth = false)
    {
        static $passed_ids;
        static $passed_actives;
        static $main_menu_id;
...
//        }
        ...
        if($depth ==1) {
            $to_print = '<div class="cbp-hrsub"><div class="cbp-hrsub-inner"> <div><' . $ul_tag . ' role="menu" '.$id_attr_str.' class="leftContent" >';
        } else {
            $to_print = '<' . $ul_tag . ' role="menu" '.$id_attr_str.' class="leftContent" >';
        }
//        $to_print = '<' . $ul_tag . ' role="menu" '.$id_attr_str.' class="{ul_class}' .' menu_' . $menu_id . ' {exteded_classes}" >';
        ...
        foreach ($q as $item) {
            ...
            if ($title != '') {
				...
				if (!isset($link) or $link == false) {
					$link = '<div itemprop="url" data-item-id="{id}" class="title" url={url}><img><span>{title}</span></div>';
				}	
                ...
                $ext_classes = trim($ext_classes);
					...
					foreach ($item as $key => $value) {
						if($key == 'icon'){
							if(!empty($value)){
                              $menu_link = str_replace('<img>', '<img src="'.$value.'">', $menu_link);
							}else{
                              $menu_link = str_replace('<img>', '', $menu_link);
							}
						}else{
                          $menu_link = str_replace('{' . $key . '}', $value, $menu_link);
						}
                ...
                if (in_array($item['parent_id'], $passed_ids) == false) {
                    if ($maxdepth == false) {
                        if (isset($params) and is_array($params)) {
                            ...
                            $test1 = $this->get_dropdown_menus($menu_params);

                        } else {
                            $test1 = $this->get_dropdown_menus($item['id']);
                        }
                    } else {
...
//        $to_print .= '</' . $ul_tag . '>';
    ...
    public function get_dropdown_menus($menu_id, $maxdepth = false)
    {
        static $passed_ids;
        static $passed_actives;
        static $main_menu_id;
...
//        }
        ...
        if($depth ==1) {
            $to_print = '<div class="cbp-hrsub"><div class="cbp-hrsub-inner"> <div><' . $ul_tag . ' role="menu" '.$id_attr_str.' class="" >';
        } else {
            $to_print = '<' . $ul_tag . ' role="menu" '.$id_attr_str.' class="" >';
        }
//        $to_print = '<' . $ul_tag . ' role="menu" '.$id_attr_str.' class="{ul_class}' .' menu_' . $menu_id . ' {exteded_classes}" >';
        ...
        foreach ($q as $item) {
            ...
            if ($title != '') {
				...
				if (!isset($link) or $link == false) {
					$link = '<a itemprop="url" data-item-id="{id}" class=""  href="{url}"><img>{title}</a>';
				}	
