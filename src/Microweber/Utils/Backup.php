...
class Backup
{
    ...
    public function exec_restore($params = false)
    {
        ...
        if ($ext_error == true) {
            return array('error' => '无效的文件扩展名。还原文件必须是 .sql 或 .zip。');
            die();
        }
...
//                $stmt = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x80-\x9F]/u', '', $stmt);
    ...
    public function delete($params)
    {
        ...
        if ($ext != 'zip' and $ext != 'sql') {
            return array('error' => "现在允许删除 {$ext} 文件。");
        }
        if (is_file($filename)) {
            unlink($filename);

            return array('success' => "$id 已经删除");
        } else {
