...
class MailSender
{
    ...
    public function test($params)
    {
        $is_admin = is_admin();
        if ($is_admin == false) {
            return array('error' => '错误：未作为管理员登录。' . __FILE__ . __LINE__);
        }

        $email_from = mw()->option_manager->get('email_from', 'email');
        if ($email_from == false or $email_from == '') {
            return array('error' => '发件人电子邮件未设置');
        } elseif (!filter_var($email_from, FILTER_VALIDATE_EMAIL)) {
            return array('error' => '发件人电子邮件无效');
        }
