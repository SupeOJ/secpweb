...
class UserManager
{
    ...
    public function login($params)
    {
        ...
        if ($check > 5) {
            ...
            return array('error' => _e('您的IP在最后一分钟有太多失败的登录尝试，1分钟后再试一次！',true));
        }
        ...
        if ($check2 > 25) {
            return array('error' => _e('您的IP在最后十分钟有太多失败的登录尝试，10分钟后再试一次！',true));
        }
        ...
        if (isset($params['username'])) {
            if (!$params['username']) {
                return array('error' => _e('Please enter username or email',true));
            }
            $ok = Auth::attempt([
        ...
        if ($ok) {
            if (defined('MW_USER_IP') and intval(Auth::user()->is_admin) == 1) {
                ...
                if ($allowed_ips) {
                    ...
                    if (!empty($allowed_ips)) {
                        ...
                        if (!$is_allowed) {
                            $this->logout();
                            return array('error' =>  _e('You are not allowed to login from this IP address', true));
                        }
                    }
                }
            }
          ...
          return array('error' => '请输入正确的用户名和密码！', true);
    }
    ...
    public function register($params)
    {
		...
		if($email == false){return array('error' => '请填写邮箱！');}
			$pattern = '/^[a-z0-9]+([._-][a-z0-9]+)*@([0-9a-z]+\.[a-z]{2,14}(\.[a-z]{2})?)$/i';
			 if(!preg_match($pattern,$email)){
                   return array('error' => '邮箱填写错误！');
		     }
        ...
        if (!isset($params['password']) or (isset($params['password']) and ($params['password']) == '')) {
            return array('error' => _e('Please set password!',true));
        }
        ...
        if (get_option('form_show_password_confirmation', 'users') == 'y') {
            if (!isset($params['password2']) or (isset($params['password2']) and ($params['password2'] != $params['password']))) {
                return array('error' => _e('Two password entries do not match!',true));
            }
        }

        if (!isset($params['username']) and !isset($params['email'])) {
            return array('error' => _e('Please set username or email!',true));
        }
        if (!isset($params['password'])) {
            return array('error' => _e('Please set a password!',true));
        }
        ...
        if (isset($params['password']) and ($params['password']) != '') {
            if ($confirm_password != false) {
                if ($params['password'] != $confirm_password) {
                    return array('error' => _e('Password confirm does not mach password!',true));
                }
            }
            ...
            if ($email != false) {
                ...
                if (empty($user_data)) {
                    ...
                    return array('success' => _e('You have registered successfully',true));
                } else {
                    ...
                    return array('error' => _e('该用户名已经注册!',true));
                }
            }
        }
    }
    ...
    public function save($params)
    {
            ...
            if (isset($params['id']) and $params['id'] != 0) {
                ...
                if ($adm == false) {
                    ...
                    if ($is_logged == false or $is_logged == 0) {
                        return array('error' => _e('You must be logged to save user',true));
                    } elseif (intval($is_logged) == intval($params['id']) and intval($params['id']) != 0) {
                        // the user is editing their own profile
                    } else {
                        return array('error' => _e('You must be logged to as admin save this user',true));
                    }
                }
            } else {
                if (defined('MW_API_CALL') and mw_is_installed() == true) {
                    ...
                    if ($adm == false) {
                        ...
                        if (intval($params['id']) != 0 and $is_logged != $params['id']) {
                            return array('error' => _e('You must be logged save your settings',true));
                        }
                    } else {
                        if (!isset($params['id'])) {
                            $params['id'] = $this->id();
        ...
        if (isset($data_to_save['email']) and isset($data_to_save['id'])) {
            ...
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if (isset($check_existing['id']) and $check_existing['id'] != $data_to_save['id']) {
                    return array('error' => _e('User with this email already exists Try different email address!',true));
                }
            }
        }
        ...
        if ($user->validateAndFill($data_to_save)) {
            ...
            if (isset($data_to_save['username']) and $data_to_save['username'] != false and isset($data_to_save['id']) and $data_to_save['id'] != false) {
                ...
                $check_existing = $this->get_all($check_existing);
                if (isset($check_existing['id']) and $check_existing['id'] != $data_to_save['id']) {
                    return array('error' => _e('User with this username already exists Try different username!',true));
                }
            }
