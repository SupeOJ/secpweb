...
<?php include mw_includes_path() . 'api/treerenderer.php'; ?>
  ...
  mw.menu_add_new = function(){
     ...
     obj.title = $('#new_menu_name').val();
      if(obj.title == '' ||  obj.title == null ) {return false;}
...
mw.menu_delete = function($id){
  ...
  if (confirm('<?php _e('Are you sure you want to delete this menu?'); ?>') === true){
    var opdata ={};
        ...
        opdata.option_key = "menu_name";
        opdata.option_value  = '';
        $.post("<?php print api_url('save_option') ?>", opdata, function (resp) {});

     $.post("<?php print api_link('menu_delete') ?>",  data, function(resp){
        window.location.href = window.location.href;
        //mw.reload_module('dropdown_menu/admin');
        mw.reload_module_parent('dropdown_menu')//dropdown_menu 
...
//
    ...
    $.post( "<?php print api_link('content/menu_item_save'); ?>",data, function( msg ) {
           // mw.reload_module('menu');
              parent.mw.reload_module('dropdown_menu');

      mw.reload_module('menu/edit_items');mw.reload_module('dropdown_menu/edit_items');
...
<script  type="text/javascript">
  if(typeof mw.menu_save_new_item1 !== 'function'){

    mw.menu_save_new_item1 = function(selector,no_reload){
      var title = $(selector).find("input[name='title']").val(); 
      ...
      mw.form.post(selector, '<?php print api_link('content/menu_item_save'); ?>', function(){
          ...
          if(no_reload === undefined){
              mw.reload_module('menu/edit_items');
              mw.reload_module('dropdown_menu/edit_items');
          }
...
<?php
 ...
 if($menu_id == false){
   ...
   $active_menu = $menu_name = $menuss['title'];
    $menu_id = get_menus('one=1&title='.$menu_name);
...
<div class="control-group form-group">
  ...
  <button type="button" class="mw-ui-btn mw-ui-btn-invert" onclick="mw.menu_add_new()"><?php _e("Save"); ?></button>
    ...
    $active_menu =   $item['title'];
      ...
      <option <?php  if($menu_name == $item['title'] or $menu_id == $item['id']): ?> <?php  $active_menu = 'left_temp' ?> selected="selected" <?php endif; ?> value="<?php print $item['title'] ?>"><?php print ucwords(str_replace('_', ' ', $item['title'])) ?></option>
      <?php endforeach ; ?>
    </select>
...
<div id="custom_link_controller" class="mw-ui-gbox">
  ...
  <button class="mw-ui-btn mw-ui-btn-info pull-right" onclick="mw.menu_save_new_item1('#custom_link_controller');">
  <?php _e("Add to menu"); ?>
  </button>
</div>
