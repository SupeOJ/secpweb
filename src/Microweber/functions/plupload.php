...
if (!$chunks || $chunk == $chunks - 1) {
    ...
    if($is_ext == 'zip') {
        if($_REQUEST['unzip'] == 'unzip') {

            $ufilename = str_replace('.'.$is_ext,'',$fileName);
            $target_temp_path = media_base_path() . DS . $host_dir . DS . 'uploaded_temp' . DS . $ufilename;

            dr_unZip($filePath,$target_temp_path);
            $uploaded_temp = getfiles($target_temp_path);

            $uploaded_templates=array();
            if(is_array($uploaded_temp)) {
                foreach($uploaded_temp as $k=>$temp) {
                    $key = substr(strrchr($temp,'/'),1);
                    $uploaded_templates[$key] = mw()->url_manager->link_to_file($temp);
                }
            }           
  
        }
       // $filePath = $uploaded_temp;
    }
...
$rerturn['uploaded_templates'] = isset($uploaded_templates) && $uploaded_templates ? $uploaded_templates : '';
...
function dr_unZip($zipName,$toDir) {  
