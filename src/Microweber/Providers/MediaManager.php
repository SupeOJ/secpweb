...
//d($params);
    ...
    public function create_media_dir($params)
    {
        ...
        if (!isset($_REQUEST['name'])) {
            $resp = array('error' => _e('You must send new_folder parameter'));
        } else {
            ...
            if (!is_dir($fn_path)) {
                ...
                $resp = array('success' => _e('Folder ' ). $fn_path . _e(' is created'));
            } else {
                $resp = array('error' => _e('Folder ' ). $fn_new_folder_path . _e(' already exists'));
            }
        }
                ...
                if (isset($fn_remove) and trim($fn_remove) != '' and trim($fn_remove) != 'false') {
                    ...
                    if (stristr($target_path, media_base_path())) {
                        if (is_dir($target_path)) {
                            ...
                            $resp = array('success' => _e('Directory ') . $target_path . _e(' is deleted'));
                        } elseif (is_file($target_path)) {
                            unlink($target_path);
                            $resp = array('success' => _e('File ') . basename($target_path) . _e(' is deleted'));
                        } else {
                            $resp = array('error' => _e('Not valid file or folder ') . $target_path . ' ');
                        }
                    } else {
                        $resp = array('error' => _e('Not allowed to delete on ') . $target_path . ' ');
                    }
                }
            }
        }
